...
import { useSalaryHistory, useDriverBookings } from "@/app/lib/hooks/useStaff";
...
export default function StaffDetailsModal({ staff, onClose }: Props) {
  ...
  const { data: salaryHistory = [], isLoading: salaryLoading } = useSalaryHistory(staff.id);
  
  // Fetch driver bookings if the staff is a driver
  const { data: driverBookings = [], isLoading: bookingsLoading } = useDriverBookings(
    staff.role.toLowerCase() === "driver" ? staff.id : ""
  );

  const isDriver = staff.role.toLowerCase() === "driver";
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        <div className="p-6">
          ...
          {activeTab === "details" && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="font-medium">¢{Number(staff.salary).toFixed(2)}</p>
              </div>
              <div>
                    ...
                    {salaryHistory.length === 0 ? (
                      ...
                      salaryHistory.map((p: any) => (
                        <tr key={p.id} className="border-t">
                          ...
                          <td className="px-4 py-2">¢{(Number(p.overtime).toFixed(2))}</td>
                          <td className="px-4 py-2">¢{(Number(p.bonuses).toFixed(2))}</td>
                          <td className="px-4 py-2">¢{(Number(p.deductions).toFixed(2))}</td>
                          <td className="px-4 py-2 font-bold">¢{(Number(p.netSalary).toFixed(2))}</td>
          ...
          {activeTab === "bookings" && isDriver && (
            <div className="overflow-x-auto">
              {bookingsLoading ? (
                <div className="text-center py-4">Loading driver bookings...</div>
              ) : (
                <table className="min-w-full">
                  <thead>
                    <tr className="bg-gray-50 dark:bg-gray-700">
