...
export const updateInsurancePolicy = createAsyncThunk(
  ...
  async (
    ...
    try {
      const response = await apiService.updateInsurancePolicy(policyId, updates);
      console.log('updated insurance', response)
      // If insurance is expired, update vehicle status
      if (updates.status === "expired") {
        const policy = response.data.results;
      ...
      if (axios.isAxiosError(error)) {
        return rejectWithValue(
          error.response?.data?.message || "Failed to update insurance policy"
        );
      }
      return rejectWithValue("An unexpected error occurred");
    }
...
export const renewInsurancePolicy = createAsyncThunk(
  ...
  async (
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        return rejectWithValue(
          error.response?.data?.message || "Failed to renew insurance policy"
        );
      }
      return rejectWithValue("An unexpected error occurred");
    }
...
export const checkExpiringPolicies = createAsyncThunk(
  ...
  async (daysThreshold: number = 30) => {
    const response = await apiService.getExpiringPolicies(daysThreshold);
    return response.data.results;
  }
);
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchInsurancePolicies.fulfilled, (state, action) => {
        ...
        if (vehicleId) {
          state.policies = state.policies.filter(p => p.vehicleId !== vehicleId);
        }
